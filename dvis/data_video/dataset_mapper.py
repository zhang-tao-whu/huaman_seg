...
class SemanticClipDatasetMapper:
    ...
    def __init__(
        ...
        *,
        ...
        if not is_tgt:
            ...
            self.src2tgt = {}
            for k, v in src2tgt.items():
                self.src2tgt[k] = self.tgt_metadata.stuff_dataset_id_to_contiguous_id[v]
    ...
    def convertSem2Inst(self, dataset_dict):
        dataset_dict_inst = {'file_name': dataset_dict['file_name'], 'image_id': dataset_dict['image_id'],
                             'width': dataset_dict['width'], 'height': dataset_dict['height']}
        annotations = []
        ...
        if sem_seg is not None:
            ...
            for _cls in classes:
                ...
                anno = {"iscrowd": 0, "category_id": _cls, "id": _cls,
                        "bbox": np.array([0, 0, 0, 0]),
                        "bbox_mode": BoxMode.XYXY_ABS, "segmentation": np.array(sem_seg == _cls)}
    ...
    def __call__(self, dataset_dict):
        ...
        """
        dataset_dict = copy.deepcopy(dataset_dict)  # it will be modified by code below
        dataset_dict = self.convertSem2Inst(dataset_dict)
...
# class HumanSemanticClipDatasetMapper:
#     """
#     A callable which takes a COCO image which converts into multiple frames,
#     and map it into a format used by the model.
#     """
